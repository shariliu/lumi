...
grid.text("Experiment", x = 0.07, y = 0.97)
grid.text("Goal", x = 0.07, y = 0.94)
grid.text("Action", x = 0.07, y = 0.91)
grid.text("Contact Relation", x = 0.07, y = 0.88)
grid.text("Mitten", x = 0.07, y = 0.85)
grid.text("Stimuli", x = 0.07, y = 0.82)

```

**Figure 2.** Looking time in towards the efficient versus inefficient reach (bottom), and proportion looking towards the inefficient reach (top) at test across Experiments 1-5 (N=152). Labels above each panel list the experiment name (Exp. 1-5), type of action during habituation (constrained or unconstrained by a barrier), goal (state.change or pick.up), whether these actions involved contact with the object (yes.causal or no.causal), whether the actor wore a mitten (yes.mitten or no.mitten), and stimuli listed in Figure 1. Error bars around means indicate within-subjects 95% confidence intervals (bottom) and bootstrapped 95% confidence intervals (top). Individual points (top) or pairs of points (bottom) indicate data from a single participant. Horizontal bars within boxes indicate medians, and boxes indicate the middle 2 quartiles of data. Violin plots (top) indicate distribution of data, area scaled proportionally to the number of observations. 
...
In Experiment 1 (N=`r length(unique(exp1.avg$subj_id))`; `r length(unique(exp1.avg$subj_id))/2` per condition), 3-month-old infants  (M=`r exp1.info$mean` days, range=`r exp1.info$min`-`r exp1.info$max`, `r exp1.info$f` female) were randomly assigned to two conditions.  Infants in the constrained condition viewed repeated video clips of a person who reached over a barrier that prevented direct access to the object and appeared to cause an object to light up by touching it (H1).  Infants in the unconstrained condition viewed the same reaches with the barrier beyond the goal object, out of the person’s way (H2). Then infants in both conditions viewed the same pair of alternating test events, in which the barrier was removed and the person reached either efficiently on a novel, direct path or inefficiently on a familiar, indirect path (T1). Across all experiments, we calculated the average looking time towards the efficient versus inefficient unobstructed reach over 3 pairs of test events. Infant looking times are often log-normally distributed (Csibra, Hernik, Mascaro, Tatone, & Lengyel, 2016), including in this dataset (see SI), and thus were log-transformed (main analysis) or transformed to proportions (meta-analysis, see SI) prior to analysis.  

A mixed effects model, including a fixed interaction between habituation and test event and a random intercept for participant identity, revealed that infants responded differently to the test events across the two conditions,  `r report(exp1.findings, 4, 3, 2)`, 2 participants excluded on the basis of Cook’s Distance (Nieuwenhuis, te Grotenhuis, & Pelzer, 2012). Contrasts from this model revealed that when the actor’s reaches were initially constrained by a barrier (H1), infants looked longer at the inefficient action (M=`r summary.avg$look[2]`s, SE=`r summary.avg$se[2]`) than the efficient action (M=`r summary.avg$look[1]`s, SE=`r summary.avg$se[1]`) at test, `r report(contrasts.exp1, 2, 3, 2) `. Critically, this looking preference cannot be attributed to low-level responses to the curvilinear reach, because infants in the unconstrained condition (H2) showed a preference for the efficient (M=`r summary.avg$look[3]`s, SE=`r summary.avg$se[3]`) over the inefficient (M=`r summary.avg$look[4]`s, SE=`r summary.avg$se[4]`) action, `r report(contrasts.exp1, 6, 3, 2)`.
...
In Experiment 2 (N=`r length(unique(exp2.avg$subj_id))`), pre-registered at https://osf.io/a5byn/, we asked whether 3-month-old infants’ causal analysis of an action is critical to their understanding of this action as intentional. We tested whether infants’ expectations from Experiment 1 depended on their construal of the person as a causal agent, who can change the state of objects on contact, by manipulating the spatiotemporal continuity of this action: a variable that modulates causal perception in older infants and adults 9–11,28 and in computational studies of learning about objects and actions 12. Infants (M=`r exp2.info$mean` days, range=`r exp2.info$min`-`r exp2.info$max`, `r exp2.info$f` female) were habituated to videos identical to H1, except that the person’s hand stopped 50 pixels above the object (approximately 2 cm on the presentation screen), and the object illuminated after a 0.5 second delay. At test, the person reached efficiently or inefficiently in the absence of the barrier, with the same spatiotemporal gap as in habituation trials (T2). I nfants looked equally between the inefficient (M=`r summary.avg$look[6]`s, SE=`r summary.avg$se[6]`) and efficient (M=`r summary.avg$look[5]`s, SE=`r summary.avg$se[5]`) reach, `r report(exp2.findings, 2, 3, 2)`, mixed effects model with trial type as fixed effect and participants as a random intercept. Their looking patterns differed from those of infants shown causally effective reaches in the corresponding condition of Experiment 1 (H1), `r report(exp12.findings, 4, 3, 2)`, mixed effects model with fixed interaction between causality and trial type and participants as a random intercept.
...
In Experiment 4 (N=20), infants (N=`r length(unique(exp4.avg$subj_id))`), infants (M=`r exp4.info$mean` days, range=`r exp4.info$min`-`r exp4.info$max`, `r exp4.info$f` female) were habituated to and tested on events where a person reached for and picked up an object identical to those from Experiments 1-3 while wearing the same mitten (H4, T3), as in SCS. In Experiment 5 (N=`r length(unique(exp5.avg$subj_id))`), infants (M=`r exp5.info$mean` days, range=`r exp5.info$min`-`r exp5.info$max`, `r exp5.info$f` female) saw almost identical videos to those from Experiment 4 except that the person reached with a bare hand (H5, T4): reaches similar to those infants commonly see in their everyday lives.

When tested on naturalistic pickup actions in Experiment 5, infants looked longer at the inefficient (M=`r summary.avg$look[14]`s, SE=`r summary.avg$se[14]`) than the efficient (M=`r summary.avg$look[13]`s, SE=`r summary.avg$se[13]`) reach of the bare hand, `r report(contrasts.exp45, 1, 3, 1)`, but in Experiment 4, they did not distinguish the inefficient (M=`r summary.avg$look[12]`s, SE=`r summary.avg$se[12]`) from the efficient (M=`r summary.avg$look[11]`s, SE=`r summary.avg$se[11]`) action of the mittened hand `r report(contrasts.exp45, 6, 3, 1)`. 

We also compared these results against those from SCS Experiment 3, wherein infants received no mittens training and viewed a person reaching with a mittened hand. The results of Experiment 5 (no mitten) differed from those in SCS Experiment 3 (mitten), `r report(exp5.scs.findings, 4, 3, 2)`, mixed effects model with fixed interaction between experiment and test event and random intercept for participants, one influential participant excluded on the basis of Cook's Distance, whereas the results from Experiment 4 (mitten) only marginally differed from those in SCS Experiment 3 (mitten), `r report(exp4.scs.findings, 4, 3, 2)`, mixed effects model with fixed interaction between experiment and test events and random intercept for participants, 2 influential participants excluded on the basis of Cook's Distance. However, infants’ looking patterns in Experiments 4 and 5 did not differ from each other, `r report(exp45.findings, 4, 3, 2)`, mixed effects model with fixed interaction between mitten and test trial, random intercept for participants, excluding 3 influential participants on the basis of Cook's Distance. Thus, the differences in infants’ responses to gloved and ungloved hands merit further investigation.
...
grid.text("Training", x = 0.05, y = .95)
grid.text("Goal", x = 0.05, y = 0.89)
grid.text("Action", x = 0.05, y = .92)
grid.text("Contact Relation", x = 0.048, y = 0.861)
grid.text("Mitten", x = 0.05, y = 0.832)
grid.text("Stimuli", x = 0.05, y = 0.805)

```

**Figure S1**. Looking time in towards the efficient versus inefficient reach (bottom), and proportion looking towards the inefficient reach (top) at test across Experiments 1-5 (n=152) and across Experiments 1-5 in Skerry et al. (2013) (n=112). Labels above each panel list the experiment name (Exp. 1-5, SCS Exp. 1-5), type of motor training (none, ineffective non-sticky mittens, or effective sticky mittens), type of action during habituation (constrained or unconstrained by a barrier), goal (state.change or pick.up), whether actions resulted in contact with the object (yes.causal or no.causal), whether the actor wore a mitten (yes.mitten or no.mitten), and stimuli listed in Figure 1. Error bars around means indicate within-subjects 95% confidence intervals (bottom) and bootstrapped 95% confidence intervals (top). Individual points (top) or pairs of points (bottom) indicate data from a single participant. Horizontal bars within boxes indicate medians, and boxes indicate the middle 2 quartiles of data. Violin plots (top) indicate distribution of data, area scaled proportionally to the number of observations.
...
**Figure S2.** Effect plots for model investigating predictors of sensitivity to action efficiency across Experiments 1-5 and Skerry et al. (2013) (total N=264, 247 included in final analysis, 17 excluded on the basis of Cook’s Distance). Each point shows estimates of effects at each level of all categorical predictors: Type of motor training (none, ineffective non-sticky mittens, or effective sticky mittens), the goal of the actor (state change vs pick up), action during habituation (constrained or unconstrained by a barrier), whether actions resulted in contact with the object (yes.causal or no.causal), whether the actor wore a mitten (yes.mitten or no.mitten). Error bars indicate 95% confidence intervals. See Table S1 for full results.

***

***
**Table S1.** Regression table for model investigating predictors of sensitivity to action efficiency across Experiment 1-5 and all experiments from Skerry et al. (2013) (total N=`r length(unique(lumi.wide$subj_id))`, `r length(unique(lumi.wide$subj_id))-meta.exclude.n` included in final analysis, `r meta.exclude.n` excluded on the basis of Cook's Distance). Dependent measure is proportion looking towards the inefficient reach, averaged across 3 test trials during test. Categorical predictors were coded using sum contrasts, and fixed effects from the model should therefore be interpreted with respect to the grand mean (i.e. with respect to 0). Degrees of freedom were generated using the Satterthwaite approximation method. Model formula: `prop.ineff.all ~ training + goal + hab + causal + mitten + (1|experiment) + (1|ageday) + (1|sex) + (1|paper)`.
...
**Figure S4.** Discrepancy between original and new coding of looking times during test trials in seconds. 
...
**Figure S6.** Looking time in seconds during each habituation trial across Experiment 1-5. Curves with 95% confidence interval ribbons indicate smoothed conditional means, generated using the loess method. Connected points indicate data from a single participant.
...
exp1.efindings <- data.frame(cbind(emodel.exp1.betas, emodel.exp1.table, emodel.exp1.ci))
names(exp1.efindings) <- c("beta", "b", "se", "df", "t", "p", "lower", "upper")
rownames(exp1.efindings) <- c("Intercept", "Habituation", "Test Event", "First Test Event", "Attention During Habituation", "Sex", "Age in Days", "Habituation:Test Event", "Habituation:First Test Event", "Test Event:First Test Event", "Habituation:Test Event:First Test Event")
...
exp2.efindings <- data.frame(cbind(emodel.exp2.betas, emodel.exp2.table, emodel.exp2.ci))
names(exp2.efindings) <- c("beta", "b", "se", "df", "t", "p", "lower", "upper")
rownames(exp2.efindings) <- c("Intercept", "Test Event", "First Test Event", "Attention During Habituation", "Sex", "Age in Days", "Test Event:First Test Event")
...
emodel.exp3.ci <- gen.ci(emodel.exp3)[3:13,]
...
exp3.efindings <- data.frame(cbind(emodel.exp3.betas, emodel.exp3.table, emodel.exp3.ci))
names(exp3.efindings) <- c("beta", "b", "se", "df", "t", "p", "lower", "upper")
rownames(exp3.efindings) <- c("Intercept", "Causal", "Test Event", "First Test Event", "Attention During Habituation", "Sex", "Age in Days", "Causal:Test Event", "Causal:First Test Event", "Test Event: First Test Event", "Causal:Test Event:First Test Event")
...
exp4.efindings <- data.frame(cbind(emodel.exp4.betas, emodel.exp4.table, emodel.exp4.ci))
names(exp4.efindings) <- c("beta", "b", "se", "df", "t", "p", "lower", "upper")
rownames(exp4.efindings) <- c("Intercept", "Test Event", "First Test Event", "Attention During Habituation", "Sex", "Age in Days", "Test Event:First Test Event")
...
exp5.efindings <- data.frame(cbind(emodel.exp5.betas, emodel.exp5.table, emodel.exp5.ci))
names(exp5.efindings) <- c("beta", "b", "se", "df", "t", "p", "lower", "upper")
rownames(exp5.efindings) <- c("Intercept", "Test Event", "First Test Event", "Attention During Habituation", "Sex", "Age in Days", "Test Event:First Test Event")
...
Regression tables (Tables S4-8) and effect plots (Figures S7-S11) for each of these models are below. In general, we found no effect of sex or age, and for all experiments, we found that infants who looked longer during habituation also were more attentive during test.  In addition, the infants in Experiment 1 who were randomly assigned to habituate to a constrained action look longer overall at test, `r report(exp1.efindings, 2, 3, 2)`. In Experiments 1 and 2, we also found that infants randomly assigned to view the efficient reach first looked longer overall at test. No other significant effects emerged.
...
**Figures S7-S11.** Effect plots for model investigating exploratory and hypothesis-driven predictors of sensitivity to action efficiency across Experiments 1-5. Each point shows estimates of effects at each level of all categorical predictors. Error bars and ribbons indicate 95% confidence intervals. See Tables S3-S7 for full results.

### Experiment 1

Model formula: `loglook ~ hab * type * first.test + total_hab + sex + ageday + (1|subj_id)`

```{r, include = TRUE, fig.height=8, fig.width = 12}
kable(exp1.efindings, col.names = c("Standardized Estimate (ß)", "Estimate (B)", "Standard Error (SE)", "df", "t", "p", "95% CI (Lower)", "95% CI (Upper)"))

plot(allEffects(emodel.exp1))
```
...
```{r, include = TRUE, fig.height=8, fig.width = 12}
kable(exp2.efindings, col.names = c("Standardized Estimate (ß)", "Estimate (B)", "Standard Error (SE)", "df", "t", "p", "95% CI (Lower)", "95% CI (Upper)"))

plot(allEffects(emodel.exp2))
...
```{r, include = TRUE, fig.height=8, fig.width = 12}
kable(exp3.efindings, col.names = c("Standardized Estimate (ß)", "Estimate (B)", "Standard Error (SE)", "df", "t", "p", "95% CI (Lower)", "95% CI (Upper)"))

plot(allEffects(emodel.exp3))
...
```{r, include = TRUE, fig.height=8, fig.width = 12}
kable(exp4.efindings,col.names = c("Standardized Estimate (ß)", "Estimate (B)", "Standard Error (SE)", "df", "t", "p", "95% CI (Lower)", "95% CI (Upper)"))

plot(allEffects(emodel.exp4))
...
```{r, include = TRUE, fig.height=8, fig.width = 12}
kable(exp5.efindings,col.names = c("Standardized Estimate (ß)", "Estimate (B)", "Standard Error (SE)", "df", "t", "p", "95% CI (Lower)", "95% CI (Upper)"))

plot(allEffects(emodel.exp5))
