...
op <- options(contrasts = c("contr.sum", "contr.poly"))
hab1 <- lmer(total_hab ~ ageday + sex + hab + goal + mitten + causal + (1|experiment), data = filter(lumi.wide, paper == "LBS"), REML=FALSE, control=lmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
hab1.table <- gen.m(hab1)
hab1.ci <- gen.ci(hab1)[3:10,]
