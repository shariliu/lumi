...
model1.exp12.cooks <- lmer(formula = loglook ~ type * mitten + (1|subj_id) + (1|experiment),
                     data = filter(exp12.avg, subj_id != "S4_9" & subj_id != "S5_10" & subj_id != "S5_7"), REML=FALSE)
model1.exp12.table <- gen.m(model1.exp12.cooks)
model1.exp12.ci <- gen.ci(model1.exp12.cooks)[4:7,]

# standardized values
model1.exp12.beta <- lmer(formula = scale(loglook) ~ type * mitten + (1|subj_id) + (1|experiment),
                     data = filter(exp12.avg, subj_id != "S4_9" & subj_id != "S5_10" & subj_id != "S5_7"), REML=FALSE)
model1.exp12.betas <- fixef(model1.exp12.beta)

# look at contrasts within each habituation condition
contrasts.exp12 <- cbind(difflsmeans(model1.exp12.beta, test.effs="type:mitten")[[1]][,1], difflsmeans(model1.exp12.cooks, test.effs="type:mitten")[[1]])
names(contrasts.exp12) <- c("beta", "b", "se", "df", "t", "lower", "upper", "p")
```
