...
# model1.all.ci <- gen.ci(model1.all)

# address influential observations
# plot(influence(model1.all, "subj_id"), which="cook",
#      cutoff=4/length(unique(lumi.wide$subj_id)), sort=TRUE,
#      xlab="Cook´s Distance",
#      ylab="Subject ID")
...
kable(cbind(model1.all.table))
```
B1-2.

```{r meta_figure, include=TRUE, fig.width=12}
...
`r kable(cbind(emodel.exp1.table, emodel.exp1.ci))`

### Experiment 2

`r kable(cbind(emodel.exp2.table, emodel.exp2.ci))`

### Experiment 3

`r kable(cbind(emodel.exp3.table, emodel.exp3.ci))`

### Experiment 4

`r kable(cbind(emodel.exp4.table, emodel.exp4.ci))`

### Experiment 5

`r kable(cbind(emodel.exp5.table, emodel.exp5.ci))`