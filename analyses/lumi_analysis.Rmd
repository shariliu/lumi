...
report <- function(table, index, places) {
  p <- NA
  if (round(table$p[index], places) < .001) {
    p <- "<.001"
  } else {
    p <- paste("=", round(table$p[index], places), sep = "")
  }
  result <- paste("[", round(table$lower[index], places), ",", round(table$upper[index], places), "], ß=", round(table$beta[index], places), ", B(SE)=", round(table$b[index],places), "(", round(table$se[index],places), "), ", "p", p, sep = "")
  return(result)
}
```
...
This analysis confirms that first-person action experience is not the only way to enhance infants’ appreciation of the causal and intentional aspects of action. We found that infants’ expectations were stronger when the observed action was spatiotemporally continuous its effect (i.e., appeared to be causal), `r report(meta.findings, 6, 3)`, when infants received effective motor training (sticky mittens), relative to no training `r report(meta.findings, 2, 3)`, when the observed agent's actions were constrained by a barrier and were efficiently adapted to that barrier, relative to an agent whose actions followed the same trajectory but were unconstrained by any barrier and therefore were inefficient, `r report(meta.findings, 5, 3)`, and when the agent pursued a state change goal, relative to a pickup goal, `r report(meta.findings, 4, 3)`. We also found that infants’ expectations were not affected when they received ineffective motor training (non-sticky mittens), relative to no training, `r report(meta.findings, 3, 3)`, or when the actor wore a mitten, relative to no mitten `r report(meta.findings, 7, 3)`.
