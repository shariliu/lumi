...
packages <- c("tidyverse", "Hmisc", "lattice", "multcomp", "lsmeans", "schoRsch", "influence.ME", "devtools", "skimr", "simr", "lme4", "sjPlot", "effects", "lmerTest", "ICC", "compute.es", "pwr", "stringr")
...
```
**Figure S1.** Density plot of looking times during test across Experiments 1-5, and Skerry et al. (2013). An analysis of these distributions reveal that a lognormal distribution (log likelihood=`r lognormal.ll`) provides a better fit to these data than a normal distribution (log likelihood= `r normal.ll`). 
 ...
 * the action being tested was causally contingent, B=`r model1.all.table$Estimate[7]`, p=`r model1.all.table$Pr...t..[7]` 
 * infants received effective motor training, relative to no training B=`r model1.all.table$Estimate[2]`, p=`r model1.all.table$Pr...t..[2]` 
 * they were habituated to an agent whose actions were contrained, B=`r model1.all.table$Estimate[4]`, p=`r model1.all.table$Pr...t..[4]` 
 * the person pursued a state change (vs a pickup) goal, B=`r model1.all.table$Estimate[5]`, p=`r model1.all.table$Pr...t..[5]`  

We also found that infants' expectations were not affected when:  

 * they received ineffective motor training, relative to no training, B=`r model1.all.table$Estimate[3]`, p=`r model1.all.table$Pr...t..[3]`
 * the person wore a mitten, B=`r model1.all.table$Estimate[6]`, p=`r model1.all.table$Pr...t..[6]`  

This analysis is consistent with our general interpretation infants expect any intentional, rational agent to reach efficiently so long as the causal mechanism of their action is clear. Either infants with motor training or presenting them with causally transparent actions provides them with this insight.
