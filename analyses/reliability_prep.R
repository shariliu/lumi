...
lumi.wide <- read.csv("/Users/shariliu/Desktop/lumi_identifiable.csv", header = TRUE)
table(lumi.wide$reliability)

lumi.long <- lumi.wide %>%
  filter(reliability == 1) %>%
  gather(trial, look.orig, T1:T6) %>%
  select(subnum, subj_id, trial, look.orig)

lumi.long$trial <- as.numeric(str_replace_all(lumi.long$trial, 'T', ''))
lumi.long$subj_id <- factor(lumi.long$subj_id)
lumi.long$subnum <- factor(lumi.long$subnum)
